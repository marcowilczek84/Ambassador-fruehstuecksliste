-- Staging-first device access. Apply only to jeuvmhahanaulvrnasgq until release approval.
...
-- Existing permissive policies remain for staging rollback, but these
...
-- Staging previously lacked this legacy production table. Model it here
-- with synthetic data only so the production rule can be exercised.
...
create policy gm_preferences_read on public.guest_preferences for select to authenticated
 using(guest_memory_private.has_device());
create policy gm_preferences_insert on public.guest_preferences for insert to authenticated
 with check(guest_memory_private.has_device());
create policy gm_preferences_update on public.guest_preferences for update to authenticated
 using(guest_memory_private.has_device()) with check(guest_memory_private.has_device());
create policy gm_preferences_delete on public.guest_preferences for delete to authenticated
 using(guest_memory_private.has_device());
