...
3. **Production-Hotel und Rollen:** genau einen Production-Hotel-Datensatz `Ambassador Hotel Zürich` plus Standardfristen erzeugen. Zwei neue nicht-anonyme Supabase-Auth-Arbeitszugänge mit verwalteten echten E-Mail-Adressen sicher anlegen und in `gm_memberships` als SERVICE und RECEPTION zuordnen. Keine Staging-Testkonten kopieren; Passwörter über sicheren Kanal festlegen.
...
5. **Config:** Vercel Production-Variablen `AMBASSADOR_GM_PRODUCTION_URL=https://xgbbwnmqgpwxxftnjrkc.supabase.co` und `AMBASSADOR_GM_PRODUCTION_PUBLISHABLE_KEY` mit dem existierenden Production-Publishable-Key sicher einrichten. Kein privater `service_role`-Key im Client. `/api/guest-memory-config` liefert Konfiguration nur für den erwarteten Production-Host, sonst `null`.
